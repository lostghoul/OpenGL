...
public class Model implements Serializable {
    ...
    public void init(float x0, float y0, float x1, float y1,
        ...
        idPoint = idSegment = idFace = 0;
        currentScale = 1.0f;
        Point p1 = addPoint(x0, y0);
        Point p2 = addPoint(x1, y1);
        Point p3 = addPoint(x2, y2);
        Point p4 = addPoint(x3, y3);
        addSegment(p1, p2, Segment.EDGE);
        addSegment(p2, p3, Segment.EDGE);
        addSegment(p3, p4, Segment.EDGE);
        addSegment(p4, p1, Segment.EDGE);
        Face f = new Face();
        f.points.add(p1);
        f.points.add(p2);
        f.points.add(p3);
